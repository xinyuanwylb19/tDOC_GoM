# Terrestrial-aquatic DOC fluxes model 
# TAF-DOC
# Version 2
...
# Read the diver data.
'''
For short term simulations, the nitrogen and sulfer depostion can be used 
to characterize the spatial patterns of DOC export.
'''
...
# Outout results
# DOC exported, DOC buried, and DOC mineralized.
DOC_Expt = []
DOC_Burl = []
DOC_Minl = []
...
# Parameters
########################################
# Soil DOC parameters / DOC density
# Desorption model
sp1=0.1356
sp2=0.956
...
# Wetland fraction, temperature, precipitation
...
# This parameter is obtained from measurement.
MDOC = 15  
...
########################################
# Soil DOC concentration model.
# The soil DOC density is g/m2.
########################################
def SDOCM(SOC):
    SDOC = sp1*sp2*SOC
    return(SDOC)
...
def WFDOCM(W, T, P, S, N, OW, WRW, WRD):
    ...
    # DOC buried in inland waters within the watershed. 
    DOCB = burp1 * math.exp(burp2 * OW) *DOCE
    
    # DOC mineralized in inland waters within the watershed.
    DOCM = minp1 * WRW * DOCE
    
    # DOC buried and mineralized during the delivery process.
    # From the watershed outlet to the ocean.
    ...
    # The total buried DOC
    # The total mineralized DOC
    # The total DOC delivered to the ocean
    DOCBB = DOCB + DOCEB
    DOCMM = DOCM + DOCEM
...
# Iterate the estimation for each watershed.
